...
expressApp.use(express_1.default.raw({ type: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', limit: '50mb' }));
// ============================================================================
// MIDDLEWARE
// ============================================================================
/**
 ...
 */
expressApp.get('/templates/firm/:firmId', verifyToken, async (req, res) => {
    try {
 ...
 */
expressApp.get('/templates/:templateId', verifyToken, async (req, res) => {
    try {
 ...
 */
expressApp.post('/templates/upload', verifyToken, async (req, res) => {
    try {
        const userId = req.user.uid;
 ...
 */
expressApp.delete('/templates/:templateId', verifyToken, async (req, res) => {
    try {
        const { templateId } = req.params;
        const userId = req.user.uid;
        // Check user permissions (admin or lawyer only)
 ...
 */
expressApp.get('/templates/:templateId/download', verifyToken, async (req, res) => {
    try {
        const { templateId } = req.params;
        const { firmId: queryFirmId } = req.query;
        const userId = req.user.uid;
        // Get user firm ID
        const userDoc = await db.collection('users').doc(userId).get();
        const userData = userDoc.data();
...
exports.templateService = functions.https.onRequest(expressApp);
